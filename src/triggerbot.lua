...
function triggerbot.CreateMove(usercmd)
   ...
   if player.weapon.is_hitscan and settings.filter.hitscan then
      ...
      if not trace or trace.fraction >= gb.flVisibleFraction then return end

      local target = trace.entity
      if not target or target:GetHealth() <= 0 then return end
      if target:GetTeamNumber() == player.team then return end
      ...
      if centerfov >= settings.fov then
         ...
         if headfov >= settings.fov then return end
      end
   ...
   if player.weapon.is_melee then
      if settings.filter.autowrench and player.class == special_classes.engineer
      and IsBuilding(target) and target:GetTeamNumber() == player.team then
      ...
      if target:GetTeamNumber() ~= player.team then
         ...
         if settings.filter.autobackstab and player.class == special_classes.spy then
            if not player.weapon.canbackstab then return end
         end
